...
{
public:
	CPsDecode(CwindowBuf* pWBuf,CDocLog* pLog);
	~CPsDecode(void);

	void			Reset();

	bool			DecodePsd(unsigned long nPos);
	void			PhotoshopParseImageResourceBlock(unsigned long &nPos,unsigned nIndent);
...
private:
	...
	void			PhotoshopParseFileHeader(unsigned long &nPos,unsigned nIndent);
	void			PhotoshopParseColorModeSection(unsigned long &nPos,unsigned nIndent);
	void			PhotoshopParseImageResourcesSection(unsigned long &nPos,unsigned nIndent);
